<template>
  <div class="bg-gray-100 min-h-screen p-6">
    ...
    <div v-for="league in leagues" :key="league.id" class="mb-12">
      <h2 class="text-xl font-semibold mb-4">{{ league.name }}</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="(match) in getMatchesForLeagueSortedByDate(league.id)" :key="match.id"
          class="bg-white rounded-lg shadow-md p-3 overflow-hidden transition-transform duration-300">
          <div class="mb-4">
            <div class="bg-blue-600 text-white text-center py-1  rounded-md">
              {{ formatDate(match.match_date, 'dd MMM yyyy') }}
            </div>
            <button @click="openModal(match)" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded-md">Open Modal</button>
  ...
  <div v-if="selectedMatch" class="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-60">
    <div class="bg-white p-8 rounded-lg shadow-md max-w-screen-8xl">
      <div class="flex items-center justify-between">
        ...
        <div class="w-3/5 text-center">
          <p class="text-7xl font-semibold">{{ selectedMatch.home_team_score }} - {{ selectedMatch.away_team_score }}</p>
      ...
      <div class="text-center text-gray-600 my-4">{{ selectedMatch.league }}</div>
